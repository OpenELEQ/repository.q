...
class Generator:
    ...
    def Create_Zips(self,addon_id,version):
        ...
        if not os.path.exists(final_zip):
            ...
            if os.path.exists(os.path.join(addon_id,'icon.png')):
                files = os.listdir(addon_id)
                for file in files:
                    if file in copyfiles:
                        shutil.copy(os.path.join(addon_id,file),addon_folder)
            elif os.path.exists(os.path.join(addon_id,'resources','icon.png')):
                files = os.listdir(os.path.join(addon_id,'resources'))
                for file in files:
                    if file in copyfiles:
                        resources_folder = os.path.join('zips',addon_id,'resources')
                        if not os.path.exists(resources_folder):
                            os.makedirs(resources_folder)
                        shutil.copy(os.path.join(addon_id,'resources',file),os.path.join(addon_folder,'resources'))
    ...
    def _generate_addons_file(self):
        ...
        self._save_file(addons_xml.encode("utf-8"), file=os.path.join('addons.xml'))

    def _generate_md5_file(self):
        try:
            m = md5.new(open(os.path.join('addons.xml')).read()).hexdigest()
            self._save_file(m, file=os.path.join('addons.xml.md5'))
        except Exception, e:
